...
const rec = (over: Partial<ProfileRecord>): ProfileRecord => ({
  id: 1, name: 'World', role: 'world', host: 'h', port: 3306, user: 'u', database: 'd', dbcDir: '', clientDir: '', exportDir: '', lastConnectedAt: null, ...over,
});
...
describe('draftFromProfiles', () => {
  it('is an empty world draft on port 3306 with no dev when nothing is saved', () => {
    expect(draftFromProfiles([])).toEqual({
      world: { host: '', port: '3306', user: '', database: '', password: '', dbcDir: '', clientDir: '', exportDir: '' },
      dev: null,
    });
  });
  it('takes the world profile that connected last, the newest dev, and never a password', () => {
    const d = draftFromProfiles([
      ...
      rec({ id: 2, host: 'recent', dbcDir: '/data', clientDir: 'E:/WoW', exportDir: '', lastConnectedAt: '2026-09-20T00:00:00.000Z' }),
      rec({ id: 3, host: 'never' }),
      rec({ id: 4, role: 'dev', name: 'Dev (.env)', host: 'dev1' }),
      rec({ id: 5, role: 'dev', name: 'Dev', host: 'dev2', port: 3307 }),
    ]);
    expect(d.world).toEqual({ id: 2, name: 'World', host: 'recent', port: '3306', user: 'u', database: 'd', password: '', dbcDir: '/data', clientDir: 'E:/WoW', exportDir: '' });
...
describe('draftToSaves', () => {
  ...
  it('omits a blank password on a saved profile and keeps its name', () => {
    ...
    expect(world).toEqual({ id: 1, name: 'World', role: 'world', host: 'h', port: 3306, user: 'u', database: 'd', dbcDir: '', clientDir: '', exportDir: '' });
  ...
  it('sends a typed password, and a blank one on a new profile, and names new rows World and Dev', () => {
    const draft: ConnectionDraft = {
      world: { host: ' h ', port: ' 3306 ', user: 'u', database: 'd', password: 'pw', dbcDir: ' /data ', clientDir: '', exportDir: ' D:/patches ' },
      dev: { ...emptyDev(), host: 'dh', user: 'du', database: 'dd' },
    };
    const { world, dev } = draftToSaves(draft, draftFromProfiles([]));
    expect(world).toEqual({ name: 'World', role: 'world', host: 'h', port: 3306, user: 'u', database: 'd', password: 'pw', dbcDir: '/data', clientDir: '', exportDir: 'D:/patches' });
...
describe('validateDraft', () => {
  const ok: ConnectionDraft = { world: { host: 'h', port: '3306', user: 'u', database: 'd', password: '', dbcDir: '', clientDir: '', exportDir: '' }, dev: null };
...
describe('worldChanged / devChanged', () => {
  ...
  it('sees a folder or password edit as a world change, and a dev edit as a dev change only', () => {
    ...
    expect(worldChanged({ ...original, world: { ...original.world, clientDir: 'E:/WoW', exportDir: '' } }, original)).toBe(true);
    expect(worldChanged({ ...original, world: { ...original.world, exportDir: 'D:/patches' } }, original)).toBe(true);
  ...
  it('ignores surrounding spaces, which are trimmed on save, but not in the password', () => {
    const padded = { ...original, world: { ...original.world, port: '3306 ', host: ` ${original.world.host}`, clientDir: ' ', exportDir: '' } };
...
describe('savedDraft', () => {
  it('takes the saved rows\u2019 IDs and names and blanks the passwords, so a retry updates the same rows', () => {
    const draft: ConnectionDraft = {
      world: { host: 'h', port: '3306', user: 'u', database: 'd', password: 'pw', dbcDir: '/data', clientDir: '', exportDir: '' },
    ...
    expect(saved).toEqual({
      world: { id: 4, name: 'World', host: 'h', port: '3306', user: 'u', database: 'd', password: '', dbcDir: '/data', clientDir: '', exportDir: '' },
