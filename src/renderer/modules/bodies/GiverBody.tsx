      ...
      {ROLES.map(({ role, list, add, tables }) => {
        ...
        return (
          <section key={role} className="entry-list" data-field={tables[0]}>
            {/* Links jump to a field by `data-field`; both relation tables of the role land here. The
                second's marker sits in the title, so it takes no room in the list. */}
            <h3 className="module-section__title">
              {list}
                ...
                <section key={i} className="entry-card" aria-label={`${list} ${n}`}>
                  <div className="entry-card__target">
                    <div className="control">
                      <label htmlFor={`${id}.kind`} className="control__label">Kind</label>
                      <select id={`${id}.kind`} aria-label={`${list} kind ${n}`} value={t.kind}
